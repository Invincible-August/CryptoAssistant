...
- **图表分析「强制刷新（实时）」7 天 1m 回补 + 聚合（Task1-4）**：`GET /api/v1/chart/bundle?source_mode=live` 在 `exchange=binance` 时会分批回补最近 7 天 Binance **1m** K 线并 upsert 覆盖入库（重复覆盖旧数据），然后基于 1m 按 UTC 自然边界聚合为请求的 `timeframe`（桶内缺任意 1m 则丢弃该桶），最后按 `limit` 截断返回。`meta` 新增 `backfill_window_days`、`aggregation_source_interval`、`dropped_total`，便于观测缺口与回补效果。聚合实现见 `backend/app/services/kline_aggregation.py`，回补实现见 `backend/app/services/kline_backfill_service.py`。

- **K 线幂等覆盖写入（Upsert）**：`market_service.save_kline()` 改为 PostgreSQL `ON CONFLICT DO UPDATE`（绑定 `uq_kline_identity`），重复导入/回补时会用新数据覆盖旧数据（覆盖 OHLCV、quote_volume、trade_count、close_time）。单测见 `tests/backend/unit/test_market_kline_upsert.py`。

- **Task4：导入进度日志（5 秒节流 + 单调）**：`MarketImportService` 增加 `TaskProgressLogger`，导入过程中持续输出包含 task name、symbol、import_type、progress(0-100%) 的日志；按 **task_id 维度** 至多每 5 秒 1 条，且 progress **单调不下降**；任务结束强制 flush 一条 **100%**。时间源抽象为可注入（可测、无真实 sleep）。单测见 `tests/backend/unit/test_market_import_progress_logging_throttle.py`。

- **监听币种自动增量更新（Task5）**：新增 `MarketAutoUpdateService`（`backend/app/services/market_auto_update_service.py`），扫描 `symbol_watches` 中 `watch_status='active'` 且 `event_type='kline'` 的记录，按“全局默认分钟数 + 单条覆盖 `config_json.update_minutes`”注册 APScheduler interval job；每次 job 仅按“已完成分钟对齐”的闭开窗口增量回补 Binance **1m** K 线并 upsert 入库。使用 Redis 分布式锁 `market:auto_update:lock:{watch_id}` 做互斥；`config_json.last_success_time` 采用单条 UPDATE 单调写回，避免竞争与回退。新增环境变量：`MARKET_AUTO_UPDATE_ENABLED`、`MARKET_AUTO_UPDATE_DEFAULT_MINUTES`、`MARKET_AUTO_UPDATE_SAFETY_LOOKBACK_MINUTES`。单元测试见 `tests/backend/unit/test_market_auto_update_service.py`（fake session/redis/scheduler，无真实 DB/Redis）。

- **前端「导入行情」页**：`frontend/src/pages/MarketImport/index.tsx` 提供表单创建 `POST /api/v1/market/import` 任务（交易对、现货/合约、UTC 日界时间范围、多选导入类型）。其中 **K 线固定按 1m 导入**，不再由前端传 `timeframe`；**持仓量/资金费率**按交易所**最小可用粒度**导入（不插值、不对齐到 1m）。创建后展示 `task_id` 并约每 2 秒轮询 `GET /api/v1/market/import/{task_id}`，显示进度条、状态标签与完成后的 `summary` / `type_results` / `errors`。API 封装见 `frontend/src/services/marketImport.ts`；侧栏菜单「导入行情」、路由 `/market-import`。需登录后访问。

- **行情导入服务与 API**：`MarketImportService`（`backend/app/services/market_import_service.py`）从 `market_import_tasks` 读取任务配置，将状态置为 `running`，按 `import_types` 拉取历史数据并写入 `market_service.save_*`。当前对外契约（UI + create schema）仅允许 `kline` / `open_interest` / `funding_rate` 三类：K 线固定 1m；OI 按最小 period（当前 5m）；资金费率按事件粒度（通常 8h），均不插值。`result_json` 含 `summary` + `type_results` + `errors`，并在 `summary.effective_granularity` 汇总有效粒度。提供 `POST /api/v1/market/import`（创建任务并后台执行）、`GET /api/v1/market/import/{task_id}`（查询状态与结果）。单元测试见 `tests/backend/unit/test_market_import_service.py`、`tests/backend/unit/test_market_import_api.py`、`tests/backend/unit/test_market_import_executor_min_granularity.py`（无网络、无真实 DB）。

- **Task3：导入执行器最小可用粒度自动降级**：`MarketImportService` 增加 import type capability table，确保 **kline 永远按 1m** 拉取、**open_interest 永远按 5m**（不插值/不对齐到 1m）、**funding_rate 按事件粒度**（通常 8h，不插值）。导入结果 `result_json.type_results.*` 写入 `effective_interval/effective_period` 等字段，并在 `result_json.summary.effective_granularity` 汇总展示。单元测试见 `tests/backend/unit/test_market_import_executor_min_granularity.py`。
