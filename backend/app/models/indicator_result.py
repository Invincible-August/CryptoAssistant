...
class IndicatorResult(Base):
    ...
    每行记录某个技术指标在特定交易对、特定时间帧下的一次计算输出，
    计算结果以 JSON 格式存储，结构由对应 IndicatorDefinition.output_schema 定义。
    """
    ...
    indicator_key: Mapped[str] = mapped_column(
        String(64), nullable=False, comment="关联的指标唯一标识"
    )
    source: Mapped[str] = mapped_column(
        String(16), nullable=False, comment="指标来源（builtin / ai）"
    )
    ...
    event_time: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, comment="指标计算对应的事件时间"
    )
    result_json: Mapped[Optional[Any]] = mapped_column(
        JSON, nullable=True, comment="指标计算输出结果（JSON格式）"
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<IndicatorResult(id={self.id}, {self.exchange}:{self.symbol} "
            f"key='{self.indicator_key}' @ {self.event_time})>"
        )
